...
-- readList :: String -> Error [LispValue]
-- readList string = case parse parser "lisp" string of
--   Left error -> throwError . ParseError $ show error
--   Right value -> return value
--   where parser :: Parser [LispValue]
--         parser = endBy parseLispValue spaces
-- 
-- load :: String -> Error [LispValue]
-- load filename = do -- Error
--   fileContent <- liftIO $ readFile filename
--   Reader.readList fileContent
